...
/* Loop through matchCommonFields and add data attr */
var mapField = function(input) {
                ...
                for (i = 0; i < matchCommonFields.length; i++) {
                  //for (var i = matchCommonFields.length - 1; i >= 0; i--) {
                   ...
                   /* Check siblings for label */
                    ...
                    if (found) {
                      addDataAttr(input, nice_name);
                      removeArrayItem(matchCommonFields, lookingFor);
                      i--; //decrement count
                    }
...
var addDataAttr = function(formInput, match){
...
var initFormMapping = function(form) {
                ...
                for (var i=0; i < form.elements.length; i++) {
                    ...
                    var inboundReturn = mapField(formInput);
...
var saveFormData = function() {
    ...
    for (var i=0; i < form.elements.length; i++) {
            console.log('inputs obj', inputsObject);
            formInput = form.elements[i];
            multiple = false;

            if (formInput.name) {

                inputName = formInput.name.replace(/\[([^\[]*)\]/g, "_array$1");
...
initFormMapping(form); /* adds data attributes */

if(form.addEventListener) {
    form.addEventListener("submit", function(evt){
        //inputsObject = {};
        evt.preventDefault();
        saveFormData(evt.target);
    }, true);
} else {
